...
export const PartListHrPlan = ({
  ...
  const [blankStoreStatus, setBlankStoreStatus] = useState(null);
      ...
      acc[index] = [
        {
          ...
          //   : "",
          plannedQtyTime: isAutoSchedule
            ? getProcessSpecialDayInfo(man.name, man.categoryId)?.isSpecialday
              ? calculatePlannedMinutes(man.hours, man.name, man.categoryId) // Job work — no qty multiplication
              : calculatePlannedMinutes(
                  quantity * man.hours,
                  man.name,
                  man.categoryId
  ...
  const calculatePlannedMinutes = (hours, processName, categoryId) => {
    ...
    if (processInfo?.isSpecialday) {
      // For special day (Job Work), treat "hours" as fraction of 24h
      return Math.round(hours * 60);
    }

    // Regular processes: convert hours to minutes normally
    return Math.round(hours * 60);
  };
  ...
  const calculateEndDate = (startDate, plannedMinutes, shift) => {
    ...
    // Calculate working minutes per day from shift data
  ...
  const calculateStartAndEndDates = (inputStartDate, plannedMinutes, shift) => {
    ...
    // Use actual shift working minutes instead of hardcoded value
    ...
    setRows((prevRows) => {
      ...
      if (isAutoSchedule && index === 0) {
        ...
        manufacturingVariables.forEach((man, processIndex) => {
          ...
          newRows[processIndex] = newRows[processIndex].map((row) => {
            ...
            if (firstAvailableMachine) {
              ...
              // Handle special day (Job Work) process as continuous 24h clock
              if (processInfo?.isSpecialday) {
                const startDateTime = new Date(startDate);
                // use existing startTime or default to shift start
                const baseStartTime =
                  row.startTime || shift?.startTime || "09:00";
                const [sH, sM] = baseStartTime.split(":").map(Number);
                startDateTime.setHours(sH, sM, 0, 0);
                // For Job Work: add minutes while skipping Sundays
        ...
        if (processInfo?.isSpecialday) {
          ...
          const startDateTime = new Date(startDate);
          const baseStartTime =
            currentRow.startTime || shift?.startTime || "09:00";
          const [sH, sM] = baseStartTime.split(":").map(Number);
          startDateTime.setHours(sH, sM, 0, 0);
          const endDateTime = addMinutesSkippingSundays(
            startDateTime,
  ...
  const addMinutesSkippingSundays = (startDateTime, minutesToAdd) => {
    if (!startDateTime || !minutesToAdd)
      return new Date(startDateTime || Date.now());

    let current = new Date(startDateTime);
    let minutesLeft = minutesToAdd;

    while (minutesLeft > 0) {
      // If current day is Sunday, move to next day start (time continues next day)
      if (getDay(current) === 0) {
        current.setDate(current.getDate() + 1);
        continue;
      }

      // Minutes remaining until the next midnight
      const nextMidnight = new Date(current);
      nextMidnight.setHours(24, 0, 0, 0);
      const minutesUntilMidnight = Math.max(
        0,
        Math.floor((nextMidnight - current) / (1000 * 60))
      );

      if (minutesLeft <= minutesUntilMidnight) {
        current = new Date(current.getTime() + minutesLeft * 60000);
        minutesLeft = 0;
        break;
      }

      // Consume current day's remaining minutes and advance to next day 00:00
      minutesLeft -= minutesUntilMidnight;
      current = nextMidnight;

      // Skip any full Sundays encountered at midnight
      while (getDay(current) === 0) {
        current.setDate(current.getDate() + 1);
      }
    }

    // Safety: if we somehow land exactly on a Sunday, move to Monday
    if (getDay(current) === 0) {
      current.setDate(current.getDate() + 1);
    }
  ...
  const calculateEndDateWithDowntime = (
    ...
    if (specialDayInfo?.isSpecialday) {
      ...
      while (workingDays < daysNeeded) {
        // If it's Sunday → skip (don’t count, move to next day)
        ...
        // Move to next day if more working days remain
        if (workingDays < daysNeeded) {
          currentDay.setDate(currentDay.getDate() + 1);
        }
      }

      // Final safety: if last day happens to fall on a Sunday, move one more day
      if (getDay(currentDay) === 0) {
        currentDay.setDate(currentDay.getDate() + 1);
      }
    ...
    // Regular process logic - use actual shift working minutes
    ...
    setRows((prevRows) => ({
      ...
      [index]: [
        ...(prevRows[index] || []),
        {
          ...
          quantityStatus: `Remaining: ${currentRemaining}`,
        },
      ],
    }));
  ...
  // Resolve selected warehouse for a given process by looking up its selected machine
  const getWarehouseForProcess = (processIndex) => {
    try {
      const processRows = rows[processIndex];
      if (!processRows || processRows.length === 0) return "";
      const selectedRow =
        processRows.find((r) => !!r.machineId) || processRows[0];
      const man = manufacturingVariables[processIndex];
      if (!man) return "";
      const machine = (machineOptions[man.categoryId] || []).find(
        (m) => m.subcategoryId === selectedRow.machineId
      );
      const resolved = machine?.wareHouse || selectedRow?.wareHouse || "";
      return resolved || "";
    } catch (e) {
      return "";
    }
  ...
  const hasAdjacentWarehouseConflict = () => {
    try {
      for (let i = 0; i < manufacturingVariables.length; i++) {
        const currentWh = getWarehouseForProcess(i);
        if (!currentWh) continue;
        const prevWh = i > 0 ? getWarehouseForProcess(i - 1) : "";
        const nextWh =
          i < manufacturingVariables.length - 1
            ? getWarehouseForProcess(i + 1)
            : "";
        if (currentWh && (currentWh === prevWh || currentWh === nextWh)) {
          const currName = `${manufacturingVariables[i]?.categoryId || ""} - ${
            manufacturingVariables[i]?.name || ""
          }`;
          const adjIndex = currentWh === prevWh ? i - 1 : i + 1;
          const adjName = `${
            manufacturingVariables[adjIndex]?.categoryId || ""
          } - ${manufacturingVariables[adjIndex]?.name || ""}`;
          toast.error(
            `Warehouse must differ between processes. "${currName}" and "${adjName}" currently use the same warehouse (${currentWh}).`
          );
          return true;
        }
      }
  ...
  const handleSubmit = async () => {
    try {
      // Block submission if adjacent processes share the same warehouse
      if (hasAdjacentWarehouseConflict()) {
        toast.error(
          "Fix warehouse conflicts between adjacent processes before submitting."
        );
        return;
      }
      ...
      if (Array.isArray(firstProcessRows)) {
        for (const r of firstProcessRows) {
          ...
          let candidate =
            (r && (r.warehouseId || r.wareHouseId || r.wareHouse)) || "";
          ...
          if (r && r.machineId) {
            const firstProcessCategoryId =
              Array.isArray(manufacturingVariables) &&
              manufacturingVariables.length > 0
                ? manufacturingVariables[0]?.categoryId
                : undefined;
            const machinesForProcess =
              firstProcessCategoryId && machineOptions[firstProcessCategoryId]
      ...
      if (!whsCode) {
        toast.error(
          "Warehouse not set for the first process. Ensure start date and machine are selected."
        );
        return;
      }
      ...
      try {
        ...
        try {
          ...
          console.error(
            "Failed to update StoreVariable for initial transfer",
            storeVarErr
          );
          // Non-blocking: inventory already posted; continue to allocations
        }
      } catch (invErr) {
        const errMsg =
          invErr?.response?.data?.error ||
          invErr?.message ||
  ...
  const calculateEndTime = (startTime, plannedMinutes, shift) => {
    ...
    if (specialDayInfo?.isSpecialday) {
      ...
      // For special day processes, use the actual shift working minutes
      const workingMinutesPerDay = shift?.workingMinutes || 480;

      // If it's exactly 1440 minutes (1 day), set end time to next day's start time
      if (plannedMinutes === 1440) {
        // Move exactly 24h forward skipping any Sundays crossed
        const startDateTime = new Date(shiftStart);
        const endDateTime = addMinutesSkippingSundays(
          startDateTime,
          plannedMinutes
        );
      ...
      // For other durations, calculate based on shift working minutes
      ...
      if (daysNeeded === 1) {
        ...
        const [shiftEndHour, shiftEndMin] = shift.endTime
          ...
          .map(Number);
        if (
          endTime.getHours() > shiftEndHour ||
          (endTime.getHours() === shiftEndHour &&
            endTime.getMinutes() > shiftEndMin)
        ) {
          endTime.setHours(shiftEndHour, shiftEndMin, 0, 0);
        }
      ...
      const endDateTime = addMinutesSkippingSundays(endTime, remainingMinutes);
  ...
  // Compute end date using both start date and start time, honoring shift and breaks
  const calculateEndDateFromStart = (
    ...
    if (!startDate || !startTime || !plannedMinutes || !shift)
    ...
    while (minutesLeft > 0) {
      ...
      if (shift.breakStartTime && shift.breakEndTime) {
        ...
        breakStart = new Date(currentDay);
        breakStart.setHours(bsH, bsM, 0, 0);
        breakEnd = new Date(currentDay);
        breakEnd.setHours(beH, beM, 0, 0);
      }

      // Normalize current within the working window
      if (current < shiftStart) current = new Date(shiftStart);
      if (
        breakStart &&
        breakEnd &&
        current >= breakStart &&
      ...
      // Work before break
      if (breakStart && current < breakStart) {
        const beforeBreak = calculateWorkMinutes(current, breakStart);
        if (minutesLeft <= beforeBreak) {
          // finishes before break today
          return formatDateUTC(currentDay);
        }
        minutesLeft -= beforeBreak;
        current = new Date(breakEnd || breakStart);
      }

      // Work after break until shift end
      const afterBreak = calculateWorkMinutes(current, shiftEnd);
      if (minutesLeft <= afterBreak) {
        // finishes today after break
        return formatDateUTC(currentDay);
      }
      minutesLeft -= afterBreak;

      // Move to next working day at shift start
      currentDay.setDate(currentDay.getDate() + 1);
      currentDay = getNextWorkingDay(currentDay);
      current = new Date(currentDay);
      current.setHours(shiftStartHour, shiftStartMin, 0, 0);
  ...
  const handleTimeChange = (index, rowIndex, newStartTime) => {
    ...
    if (specialDayInfo?.isSpecialday) {
      ...
      newEndDate = calculateEndDateFromStart(
        row.startDate,
        newStartTime,
        row.plannedQtyTime,
        shift
    ...
    setRows((prevRows) => {
      ...
      for (let p = index + 1; p < manufacturingVariables.length; p++) {
        ...
        const thisShift =
          shiftOptions.find((s) => s.name === currentNext.shift) || shift;
  ...
  const openApproveModal = () => {
    ...
    if (blankStoreError) {
      toast.error(
        "Cannot approve: Failed to fetch blank store quantity. Please retry."
      );
      return;
    }
              ...
              {blankStoreError && (
                ...
                >
                  <div style={{ fontSize: "12px", color: "#dc2626" }}>
                    ❌ Failed to fetch blank store quantity for ItemCode:{" "}
                    {partsCodeId}. Click refresh to retry.
                  </div>
                </div>
              )}
              {!isLoadingBlankStore &&
                ...
                blankStoreStatus === "found" && (
                  ...
                  >
                    <div style={{ fontSize: "12px", color: "#065f46" }}>
                      ✅ Blank store quantity loaded from API (ItemCode:{" "}
                      {partsCodeId})
                    </div>
                  </div>
                )}
                                    ...
                                    setRows((prevRows) => {
                                      ...
                                      if (isAutoSchedule && index === 0) {
                                        ...
                                        for (
                                          ...
                                          const processRows = updatedRows[p];
                                          if (
                                            !processRows ||
                              ...
                              >
                                ...
                                <Autocomplete
                                  ...
                                  onChange={(event, newValue) => {
                                    ...
                                    // Enforce different warehouse than adjacent processes
                                    try {
                                      const currentWarehouse =
                                        newValue?.wareHouse || "";
                                      const prevWarehouse =
                                        index > 0
                                          ? getWarehouseForProcess(index - 1)
                                          : "";
                                      const nextWarehouse =
                                        index <
                                      ...
                                      if (
                                        ...
                                        toast.error(
                                          "Warehouse must differ from adjacent processes. Choose another machine/warehouse."
                                        );
                                        return;
                                      }
                                    } catch (_) {}
                                    ...
                                    setRows((prevRows) => {
                                      ...
                                      if (
                                        ...
                                        // For Job Work (special day), compute using continuous minutes skipping Sundays
                                        const currentProcessInfo =
                                          manufacturingVariables[index];
                                        const isJobWork =
                                          !!getProcessSpecialDayInfo(
                                        ...
                                        if (isJobWork) {
                                          ...
                                          // For regular processes, end time still derived from shift rules
                                          updatedRows[rowIndex].endTime =
                                            calculateEndTime(
                                              updatedRows[rowIndex].startTime ||
                                                shift?.startTime ||
        ...
        <ModalFooter>
          <Button
            ...
            onClick={async () => {
              ...
              try {
                await handleSubmit(); // assume this is async
                setIsConfirmationModalOpen(false); // close only after success
              } catch (err) {
                console.error(err);
      ...
      >
        ...
        <ModalBody>
          ...
          {isLoadingBlankStore ? (
            ...
            <p style={{ color: "#dc2626" }}>
              Cannot approve: ItemCode {partsCodeId} found but not in BLNK
              warehouse.
            </p>
          ) : quantity <= blankStoreQty ? (
            <p style={{ color: "green" }}>
