...
export const PartListHrPlan = ({
  ...
  const calculateEndDateFromStart = (startDate, startTime, plannedMinutes, shift) => {
    if (!startDate || !startTime || !plannedMinutes || !shift) return formatDateUTC(startDate);
  ...
  const handleTimeChange = (index, rowIndex, newStartTime) => {
    ...
    setRows((prevRows) => {
      ...
      for (let p = index + 1; p < manufacturingVariables.length; p++) {
        ...
        const machineForProcess = (machineOptions[currentProcessInfo.categoryId] || []).find(
          (m) => m.subcategoryId === currentNext.machineId
        );

        // Recalculate end date and time for this process
        // End date should be derived from both start date & time to advance across days accurately
        const recomputedEndDate = calculateEndDateFromStart(
          formatDateUTC(nextStartDateObj),
          nextStartTime,
          currentNext.plannedQtyTime,
          thisShift
        );
        const recomputedEndTime = calculateEndTime(
          nextStartTime,
          currentNext.plannedQtyTime,
          thisShift
        );
...
//   // Ensure start date/time respects shift boundaries; if startTime is past shift end,
//   // bump date to next working day and reset to shift start.
//   const adjustStartForShiftBounds = (inputDate, inputTime, shift) => {
//     if (!shift || !inputDate || !inputTime) {
//       return { date: inputDate, time: inputTime };
//     }

//     const [startHour, startMin] = String(inputTime).split(":").map(Number);
//     const [shiftEndHour, shiftEndMin] = String(shift.endTime || "17:30")
//       .split(":")
//       .map(Number);
//     const [shiftStartHour, shiftStartMin] = String(shift.startTime || "09:00")
//       .split(":")
//       .map(Number);
...
//     let timeStr = inputTime;

//     if (startMinutes > shiftEndMinutes) {
//       // move to next working day and reset to shift start
//       const nextDay = getNextWorkingDay(new Date(dateObj.setDate(dateObj.getDate() + 1)));
//       dateObj = new Date(nextDay);
//       timeStr = `${String(shiftStartHour).padStart(2, "0")}:${String(
//         shiftStartMin
//       ).padStart(2, "0")}`;
//     }
...
//               // If the computed start time spills past the shift end, bump to next working day
//               const adjusted = adjustStartForShiftBounds(startDate, startTime, shift);
//               startDate = adjusted.date;
//               startTime = adjusted.time;
//             }
...
//           // Determine shift for next process; fallback to first shift option
//           const nextShift =
//             shiftOptions.find((s) => s.name === nextProcess.shift) ||
//             (shiftOptions.length > 0 ? shiftOptions[0] : null);

//           // Start from current process end date by default
//           let proposedStartDate = new Date(endDate);
//           let proposedStartTime = nextStartTime;

//           // If proposed time exceeds shift end, move to next working day and reset time
//           const adjusted = adjustStartForShiftBounds(
//             proposedStartDate,
//             proposedStartTime,
//             nextShift
//           );
...
//             ...nextProcess,
//             startTime: adjusted.time,
//             startDate: formatDateUTC(adjusted.date),
//           };
//         }
//       }
