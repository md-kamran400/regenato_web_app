...
//     const totalHours =
//       processPartsMap[processName]?.reduce(
  ...
  <table className="table align-middle table-nowrap">
    ...
    <tbody>
      {manufacturingVariables
        .filter((variable) => processPartsMap[variable.name]?.length > 0)
        .map((variable) => (
          <React.Fragment key={variable._id}>
            <tr onClick={() => handleRowClick(variable.name)}>
              <td>{variable.name}</td>
              ...
              <td>
                <Input
                  ...
                  onChange={(e) =>
                    handleInputChange(variable.name, "machineHours", e.target.value)
                  }
                />
              </td>
              <td>
                <Input
                  ...
                  value={inputs.machineTbu[variable.name] || ""}
                  onChange={(e) =>
                    handleInputChange(variable.name, "machineTbu", e.target.value)
                  }
                />
              </td>
              <td>
                <Input
                  ...
                  value={inputs.daysToWork[variable.name] || ""}
                  onChange={(e) =>
                    handleInputChange(variable.name, "daysToWork", e.target.value)
                  }
                />
              </td>
              <td>{calculatedValues[variable.name] || 0}</td>
                ...
                <td colSpan={8}>
                  <div className="table-responsive">
                    <table className="table align-middle table-nowrap">
                      ...
                      <tbody>
                        {processPartsMap[variable.name]?.map((part) => (
                          <tr key={part.partName}>
                            <td>{part.partName}</td>
                            <td>{part.quantity}</td>
                            <td>{part.hours}</td>
        ...
        <Modal isOpen={modalAdd} toggle={toggleAddModal}>
          ...
          <ModalBody>
            <form onSubmit={handleSubmit}>
              <Autocomplete options={parts}getOptionLabel={(option) => option.partName}onChange={handleAutocompleteChange}renderInput={(params) => (
                  <TextField {...params} label="Select Part" />
                )}
              />
              <div className="mb-3">
                ...
                <Input className="form-control" type="number" id="quantity" value={quantity} onChange={(e) => setQuantity(e.target.value)} required
                />
              </div>
