...
const AssmblyMultyPart = ({
  ...
  useEffect(() => {
    ...
    fetchPartsListItems();
  }, [_id, partsAssmeblyItems, assemblyId]);
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      const updatedPartsListItems = [
        ...(partsAssmeblyItems.partsListItems || []),
        newPart,
      ];
  ...
  const fetchUpdatedParts = async () => {
    try {
      const response = await fetch(
        `${process.env.REACT_APP_BASE_URL}/api/projects/${_id}/assemblyPartsLists/${assemblyId}/assemblyMultyPartsList/${partsAssmeblyItems._id}`
      );

      if (!response.ok) {
        throw new Error("Failed to fetch updated parts");
      }

      const updatedParts = await response.json();

      // Call a prop function to update the parent state
      onUpdateParts(updatedParts);
    } catch (error) {
      console.error("Error fetching updated parts:", error);
    }
  };
  ...
  return (
    <>
      ...
      <Col lg={12} >
        <Row>
          <Col lg={12}>
            <Card style={{ boxSizing: "border-box", borderTop: "5px solid #8E24AA" }}>
...
//   // fetching
//   const fetchPartsListItems = useCallback(async () => {
//     try {
//       const response = await fetch(
//         `${process.env.REACT_APP_BASE_URL}/api/projects/${_id}/assemblyPartsLists/${assemblyId}/assemblyMultyPartsList/${partsAssmeblyItems._id}/items`
//       );
