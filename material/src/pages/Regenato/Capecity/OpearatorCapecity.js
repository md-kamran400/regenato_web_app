...
const OperatorCapacity = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      // Process allocations data - FIXED: Handle nested allocations structure
      const processedAllocations = allocationsRes.data.map((project) => {
        ...
        return {
          ...project,
          flatAllocations: flatAllocations
        };
      });
      ...
      // Calculate initial occupied operators - FIXED
      const occupiedOperatorNames = new Set();
      processedAllocations.forEach((project) => {
        project.flatAllocations.forEach((allocation) => {
          if (allocation.operator && allocation.operator.trim() !== "") {
            occupiedOperatorNames.add(allocation.operator.trim());
          }
        });
      });
      
      setOccupiedOperators(occupiedOperatorNames.size);
      console.log("Occupied operators:", Array.from(occupiedOperatorNames));
  ...
  const calculateCategoryStats = () => {
    ...
    // Second pass: identify occupied operators across all categories - FIXED
    filteredAllocations.forEach((project) => {
      project.flatAllocations.forEach((allocation) => {
  ...
  const filterAllocationsByDate = () => {
    ...
    if (!startDate || !endDate || startDate.getTime() === endDate.getTime()) {
      ...
      // Recalculate occupied operators - FIXED
      const occupiedOperatorNames = new Set();
      allocations.forEach((project) => {
        project.flatAllocations.forEach((allocation) => {
          if (allocation.operator && allocation.operator.trim() !== "") {
    ...
    const filtered = allocations
      .map((project) => {
        const filteredFlatAllocations = project.flatAllocations.filter((allocation) => {
          const allocStartDate = safeParseDate(allocation.startDate);
          const allocEndDate = safeParseDate(allocation.endDate);

          if (!allocStartDate || !allocEndDate) return false;
        ...
        return {
          ...project,
          flatAllocations: filteredFlatAllocations
        };
      })
      .filter((project) => project.flatAllocations.length > 0);

    setFilteredAllocations(filtered);

    // Count occupied operators - FIXED
    const occupiedOperatorNames = new Set();
    filtered.forEach((project) => {
      project.flatAllocations.forEach((allocation) => {
        if (allocation.operator && allocation.operator.trim() !== "") {
  ...
  const getOperatorAllocations = (operatorName, checkOccupied = false) => {
    ...
    sourceData.forEach((project) => {
      project.flatAllocations.forEach((allocation) => {
        ...
        // FIXED: Direct comparison since operator field contains just the name
        if (allocation.operator.trim() === operatorName) {
          const isWithinRange =
            !startDate ||
            !endDate ||
            startDate.getTime() === endDate.getTime() ||
            (parseISO(allocation.startDate) <= endDate &&
              parseISO(allocation.endDate) >= startDate);
          ...
          <ModalBody>
            ...
            <div className="mb-4">
              ...
              {operatorAllocations.length > 0 ? (
                ...
                <p className="text-muted">No allocations found for this operator.</p>
              )}
            </div>
          </ModalBody>
          <ModalFooter>
