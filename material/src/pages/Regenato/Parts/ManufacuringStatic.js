...
const ManufacuringStatic = ({ partDetails }) => {
  ...
  const fetchShipmentData = useCallback(async () => {
    ...
    try {
      const response = await fetch(
        `http://localhost:4040/api/parts/${partDetails._id}/manufacturingVariablesstactics`
      );
      if (!response.ok) {
  ...
  const handleEditSubmit = async (e) => {
    ...
    try {
      const response = await fetch(
        `http://localhost:4040/api/parts/${partDetails._id}/manufacturingVariablesstactics/${editId}`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
  ...
  const handleDelete = async (_id) => {
    ...
    try {
      const response = await fetch(
        `http://localhost:4040/api/parts/${partDetails._id}/manufacturingVariablesstactics/${_id}`,
        {
          method: "DELETE",
        }
      );
